...
VALUE_SEMANTICS: Dict[str, Dict[str, tuple]] = {
    ...
    "marriage_event_structure.palace_state": {
        "STABLE": ("盲派婚姻·配偶宫安稳（配偶宫=日支；案例43好婚姻组合：妻宫正财坐正位两辰拱财局）", "配偶宫安稳，婚姻基础好"),
    },
    "marriage_event_structure.marriage_state": {
        "BROKEN": ("盲派口诀·配偶宫逢冲必离婚/配偶宫破星损（案例47：日支申金被月柱未冲→配偶宫逢冲必离婚）", "婚姻上容易出现分离、难长久的问题"),
        "CHALLENGED": ("盲派婚姻·配偶宫受损（案例详解第五部分：配偶宫与配偶星相穿/相刑/相破=婚姻出问题）", "婚姻上容易有波折，需经营"),
        "HARMONIOUS": ("盲派婚姻·配偶宫稳定（案例详解第五部分：配偶宫与配偶星相生/相合/拱合=关联吉，婚姻好）", "婚姻状态平稳，配偶宫无破"),
        "UNDETERMINED": ("盲派婚姻·证据不足（fail-closed，不做断言）", "婚姻判定证据不足，不做断言"),
    },
    "marriage_event_structure.spouse_star_present": {
        "True": ("盲派婚姻·配偶星在局（案例详解第五部分：男命以财星为妻，女命以官杀为夫）", "配偶星在局中（有婚姻对象之缘）"),
    ...
    "wealth_event_structure.wealth_state": {
        "DIRECTED_AND_ESTABLISHED": ("盲派财富·有势又有功定是富贵翁（VERIFY-BLIND-024：财官在主位就是我的财官）+ 财现+财被取+做功成（案例集：食伤生财/制财做功）", "财星被定向取用且做功成立，求财有成"),
        "SUBSTITUTED_AND_ESTABLISHED": ("盲派换象·伤食当财/禄当财/官杀当财（VERIFY-BLIND-022；案例14金水伤官当财富看/案例15以禄当财/案例19卯官当财看）", "财以换象方式成立（伤食/禄/官杀当财），求财方式特别"),
        "DIRECTED_PARTIAL": ("盲派财富·财被取未制净（案例23：火与燥土之势制申金食神，只是克制效率不高）", "财星有取但未全成，求财有得有失"),
        "SUBSTITUTED_CANDIDATE": ("盲派换象·伤食当财候选（案例14：八字无财，以伤食当财富看）", "财以换象候选，求财方式待定"),
        "PRESENT_UNTAKEN": ("盲派财富·有财未被取（案例8：有财官≠有富贵，关键在'谁在做功'、'是否为我所用'）", "局中有财但未被取用，财不易到手"),
        "ABSENT_NO_SUBSTITUTION": ("盲派财富·无财亦无换象（案例25：火土有势但无大功→平常八字）", "局中无财且无换象，财源薄"),
    ...
    "wealth_event_structure.wealth_present": {
        ...
        "False": ("盲派财富·财星不现（盲派换象：八字无财，以伤食/禄当财富看——案例14/15）", "局中财星不现，以换象论财"),
    },
    # ── THEME-006 身体疾厄 ────────────────────────────────
    "body_event_candidate.candidate": {
        "LU_UNDER_ATTACK": ("盲派口诀·禄怕见绝更怕穿害（案例3：戊申己未庚申辛巳，禄神被未土脆克+巳火合克→交通意外下肢残疾）", "禄神受攻击（被穿/合克/脆金），身体或福报易受损"),
        "YANG_REN_CLASHED": ("盲派口诀·羊刃逢冲血光之灾（案例47：庚午辛未壬申癸酉，日支申金羊刃被月柱未冲→车祸断腿）", "羊刃逢冲，有血光/外伤风险"),
        "UNDETERMINED": ("盲派身体·证据不足（fail-closed，不做断言）", "身体判定证据不足，不做断言"),
    },
    "body_event_candidate.lu_attacked": {
        "True": ("盲派口诀·禄怕见绝更怕穿害（案例3：禄神被穿/合克则受损）", "禄神状态受损（穿/冲/合克）"),
        "False": ("盲派口诀·禄怕见绝更怕穿害（案例3：禄神环境正常）", "禄神未被攻击"),
    },
    "dry_earth_brittle": {
    ...
    "occupation_candidate.work_types": {
        "CONTAIN_CONTROL_OFFICER_BY_FOOD_INJURY": ("案例12（甲辰戊辰癸卯己未）：木有气势，主位卯穿制官星辰土→伤食制官局，命有官职", "含以食伤制官杀取功名，走公职/管理路线"),
        "CONTAIN_GENERATE_WEALTH_BY_FOOD_INJURY": ("案例46（戊辰己巳庚午辛未）：时柱辛未食神生财→食伤做功技术赚（医生年入百万）", "含以食伤生财，靠技艺/技术谋财"),
        "CONTAIN_TRANSFORM_OFFICER_BY_RESOURCE": ("案例23（戊申壬戌戊午壬戌）：食神主技能、印主单位→技能被单位重用（工薪族）", "含以印化官杀，靠单位/文职立足"),
    ...
    "official_event_structure.official_state": {
        ...
        "CONTROLLED_PARTIAL": ("盲派口诀·官杀制不净当财看（和珅例：年上午制不净当财看，时上午制净了当官看）", "官杀有制但制不净，功名/职位有限"),
        "DAMAGED": ("盲派口诀·伤官损官（案例2：子水伤官穿未土官库→官根受损，官场梦碎）", "官星被穿损，官场/体制内不顺"),
        "ROBBED": ("盲派口诀·官星被劫财合走非我所有（案例8：官星被宾位劫财合去→终身仓库保管员）", "官星被劫财合走，职位非我所有，难掌实权"),
        "UNCONTROLLED": ("盲派口诀·官杀无制必犯官非（案例47：官杀旺无印化无食伤制→打架判刑3年）", "官杀无制，易犯官非/与官方冲突"),
        "UNDETERMINED": ("盲派官贵·证据不足（fail-closed，不做断言）", "官贵判定证据不足，不做断言"),
    },
    "work_efficiency": {
        "LARGE": ("盲派效率·功大者贵（《盲派命理·案例资料集》核心心法：功大者贵，无功者贱，做负功者凶）", "做功效率大，事业成就层次高"),
        "MEDIUM": ("盲派效率·做功效率中（案例23：火与燥土之势制申金食神，克制效率不高）", "做功效率中等，事业成就有一定层次"),
        "SMALL": ("盲派效率·做功效率小（功小者贱/平常——《盲派命理·案例资料集》核心心法）", "做功效率小，事业成就层次低"),
    },
    # ── THEME-009 田宅家业 ────────────────────────────────
    "zuo_gong.墓库收物": {
        "EFFECTIVE": ("盲派墓库·墓库喜冲，不冲不发（案例48铁断：库不开则财官无用，一冲则发；辰库收水巨富案例1）", "墓库收物成立，家业/积蓄有成"),
        "NOT_EFFECTIVE": ("盲派墓库·墓库喜冲（案例48：库不开则财官无用）", "墓库收物未成立，家业/积蓄平平"),
        "UNDETERMINED": ("盲派墓库·证据不足（fail-closed，不做断言）", "田宅家业判定证据不足，不做断言"),
    },
    "zuo_gong.冲开墓库": {
        "冲开墓库": ("盲派墓库·墓库喜冲，不冲不发（案例48：丑未冲打开财库与杀库，制杀取财）", "墓库被冲开，家业/积蓄有变动之机"),
    },
    # ── THEME-010 福德精神 ────────────────────────────────
    "zuo_gong.食伤做功": {
    ...
    # ── THEME-011 父母长辈 ────────────────────────────────
    "parents.father(偏财)": {
        "PRESENT": ("盲派六亲·父星=偏财（案例详解：宫位比十神更准；年柱祖上父母）", "父星（偏财）在局中"),
        "ABSENT": ("盲派六亲·父星=偏财（偏财不显则父缘淡）", "父星（偏财）不显，父缘淡或助力少"),
    },
    "parents.mother(印星)": {
        "PRESENT": ("盲派六亲·母星=印星（案例详解：宫位比十神更准；年柱祖上父母）", "母星（印星）在局中"),
...
TOKEN_SEMANTICS: Dict[str, Dict[str, tuple]] = {
    "marriage_event_structure.palace_state": {
        "CLASHED": ("盲派婚姻·配偶宫逢冲（案例47：日支被月柱冲→配偶宫逢冲必离婚）", "配偶宫（日支）被冲，婚姻根基动摇"),
        "HARMED": ("盲派婚姻·配偶宫逢穿（案例详解：配偶宫与配偶星相穿=婚姻出问题）", "配偶宫（日支）被穿，暗中受克，婚姻暗损"),
        "PUNISHED": ("盲派婚姻·配偶宫逢刑（案例详解：配偶宫相刑=婚姻出问题）", "配偶宫（日支）被刑，夫妻易有口舌纠纷"),
    ...
    "zuo_gong.冲开墓库": {
        "冲开墓库": ("盲派墓库·墓库喜冲，不冲不发（案例48：库不开则财官无用，一冲则发）", "墓库被冲开，家业/积蓄有变动之机"),
    },
}
...
EVENT_SEMANTICS: Dict[str, tuple] = {
    "MARRIAGE_BROKEN": ("盲派口诀·配偶宫逢冲必离婚/配偶宫破星损（案例47，BLIND-DJ-004）", "婚姻容易出现分离、难长久的问题"),
    "MARRIAGE_CHALLENGED": ("盲派婚姻·配偶宫受损（案例详解：配偶宫与配偶星相穿相刑相破=婚姻出问题，BLIND-DJ-004）", "婚姻有波折，需用心经营"),
    "MARRIAGE_STABLE": ("盲派婚姻·配偶宫安稳（案例43：妻宫正财坐正位两辰拱财局→婚姻好，BLIND-DJ-004）", "婚姻状态平稳"),
    "WEALTH_ESTABLISHED": ("盲派财富·有势又有功定是富贵翁（VERIFY-BLIND-024；BLIND-DJ-005/007/009）", "求财有成，财富能到手"),
    "WEALTH_CANDIDATE": ("盲派财富·财被取未全成（案例23：克制效率不高，BLIND-DJ-005/007）", "求财有得有失，财富待定"),
...
TIME_KIND_SEMANTICS: Dict[str, tuple] = {
    "CHONG": ("盲派应期·逢冲则动（VERIFY-BLIND-010：冲是做功方式之一；案例40午冲子得升迁）", "大运/流年冲引动原局，主变动"),
    "CHUAN": ("盲派应期·穿比冲更狠（案例3：禄怕见绝更怕穿害；穿=背后偷袭暗中受克）", "大运/流年穿引动原局，暗中受损"),
    "SANXING": ("盲派应期·三刑引动（案例15：19岁癸丑年丑未戌三刑应期）", "大运/流年刑引动原局，主口舌纠纷"),
    "FANYIN": ("盲派应期·反吟：天克地冲（VERIFY-BLIND-027）", "大运/流年反吟（天克地冲）引动原局，主剧烈变动"),
    "FUYIN": ("盲派应期·伏吟：重复引动（VERIFY-BLIND-027；案例9庚戌大运伏吟日支戌土）", "大运/流年伏吟引动原局，主原局结构重演/加重"),
    "LIUHE": ("盲派应期·六合引动（案例12：壬申运卯申合，卯有功财运大好）", "大运/流年合引动原局，主合绊/牵动"),
    "SANHE": ("盲派应期·三合引动（案例49：壬午运寅午戌三合火局制子水→权力巅峰）", "大运/流年三合引动原局，主成局/聚势"),
    "MUKU_KAI": ("盲派应期·墓库逢冲则开（案例48：丑未冲开财库与杀库；BLIND-DJ-005）", "大运/流年冲开墓库，主积蓄变动/库开"),
    "LU": ("盲派应期·禄神引动（案例3：乙卯运庚寅年冲禄三刑→禄神受损；BLIND-DJ-002）", "大运/流年引动禄神，主身体/福报相关变动"),
    "TOUGAN": ("盲派应期·遁藏透干应期（VERIFY-BLIND-028：地支遁藏字在大运/流年天干出现=该字应期；案例12甲运）", "大运/流年透干引动原局，主天干层面变动"),
    "ZIXING": ("盲派应期·自刑引动（案例15：未戌刑、丑未戌三刑应期）", "大运/流年自刑引动原局，主自我消耗/口舌"),
    "ZIZAIXIAN": ("盲派应期·字再现引动（VERIFY-BLIND-028：原局字再现=应期；案例9庚戌运伏吟日支戌土）", "大运/流年原局字再现引动，主原局重现"),
}
...
def audit_assertion_provenance() -> Dict:
    ...
    规则：
    - 出处标记 = 书名（《》）/ 篇名（盲派X章/金口诀/铁断）/ 案例号（案例N）/ VERIFY 号（VERIFY-BLIND-NNN）
      / 讲义（段建业讲义）/ 心法（案例集核心心法）/ 原书（段氏理象学等）之一。
    ...
    """
    marks = ["《", "盲派", "段建业", "案例", "VERIFY", "讲义", "心法", "铁断", "金口诀",
             "渊海子平", "理象学", "命理玄机探秘", "段氏"]
