"""YHZP Facts Builder（Phase 6 §65/§B-3）。

消费 L0 Chart → 生成求值上下文 → Rule Engine → 按 output 字段分六组 facts →
attach evidence（§64）→ YHZPEngineResult（§70）。
...
from engines.yuhai_ziping.calculation.l0_adapter import build_base_view, build_contexts
...
# 其余（element/stem_he/hidden_stem/na_yin/zodiac/twelve_stage/神煞等）→ basic_structure
GEJU_CANDIDATE_FIELDS: set = set()  # YHZP 规则暂无直接格局输出，Phase 6 预留
...
class FactsBuilder:
    """L0 Chart → 六组 facts（带完整证据链）。"""

    def __init__(self, rule_engine: RuleEngine | None = None, evidence: EvidenceRegistry | None = None) -> None:
        self.engine = rule_engine or RuleEngine()
        self.evd = evidence or EvidenceRegistry()

    def build(self, l0_chart: Dict[str, Any], input_ref: str | None = None) -> YHZPEngineResult:
        ...
        def consume(run_ctxs: List[Dict[str, Any]]) -> Dict[str, str]:
            ...
            for ctx in run_ctxs:
                for raw in self.engine.run(ctx):
