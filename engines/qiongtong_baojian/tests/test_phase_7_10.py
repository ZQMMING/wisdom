"""Phase 7-10 管线测试 · qiongtong_baojian。"""
...
def test_judgment_assertions():
    from engines.qiongtong_baojian.judgment.judgment_builder import build_assertions
...
def test_golden_runner_blocked():
    from engines.qiongtong_baojian.golden.golden_runner import GoldenRunner
    gr = GoldenRunner()
    assert gr.approved_technical_goldens() == []  # TG 未审批 → 阻塞（§67 不代行）


def test_provenance_record():
    from engines.qiongtong_baojian.provenance.provenance import ProvenanceRecorder
...
def test_regression_harness():
    from engines.qiongtong_baojian.regression.regression_harness import DEMO_CHART, compare, snapshot_all
...
def test_production_admission_blocks_on_golden():
    from engines.qiongtong_baojian.production.admission import ProductionAdmission
