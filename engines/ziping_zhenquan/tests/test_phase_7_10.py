"""Phase 7-10 管线测试 · ziping_zhenquan。"""
...
def test_judgment_assertions():
    from engines.ziping_zhenquan.judgment.judgment_builder import build_assertions
...
def test_golden_runner_approved():
    from engines.ziping_zhenquan.golden.golden_runner import GoldenRunner
    gr = GoldenRunner()
    assert len(gr.approved_technical_goldens()) == 12  # Human 已整体审批（§67）


def test_provenance_record():
    from engines.ziping_zhenquan.provenance.provenance import ProvenanceRecorder
...
def test_regression_harness():
    from engines.ziping_zhenquan.regression.regression_harness import DEMO_CHART, compare, snapshot_all
...
def test_production_admission_passes():
    from engines.ziping_zhenquan.production.admission import ProductionAdmission
