...
from tongshu.ziwei.pattern import recognize_patterns, recognize_patterns_from_chart
...
class TestZiweiPattern(unittest.TestCase):
    ...
    def test_ziwei_duze(self):
        """紫微独坐 → 紫微独坐"""
        patterns = recognize_patterns(['紫微'])
        names = [p[0] for p in patterns]
        self.assertIn('紫微独坐', names)

    def test_shapolang(self):
        """七杀+破军+贪狼 → 杀破狼格"""
        patterns = recognize_patterns(['七杀', '破军', '贪狼'])
        names = [p[0] for p in patterns]
        self.assertIn('杀破狼', names)
