...
def time_index_from_hour(hour: int) -> int:
    ...
    iztro convention: 0 = 早子时(00:00-01:00), 1 = 丑时 ... 11 = 亥时
...
class ZiweiEngine:
    ...
    def native_direction_for_year(self, chart, lunar_date, hour, gender, target_year):
        ...
        修复 native_direction 的"今天"大限四化的局限：flow_year焦点年份
        可能是未过/过去年份，应取该年份所在大限的四化。
        返回 opportunity / caution / neutral。
        """
        if not chart or not chart.soul_palace_main_star:
            return "neutral"
    ...
    def _compute_via_iztro(self, lunar_date, hour, gender):
        ...
        # 取绝对值拼日期，并设 isLeapMonth 正确为 true
        ...
        // 命宫空宫时借对宫(迁移宫)主星论事, 不再留白。借星来源记soulBorrowed标志记录, 便于后续打折处理
    ...
    def flow_years_mutagen(self, years, lunar_date, hour, gender):
        """获取多个指定年份的紫微流年四化（yearly mutagen，[禄权科忌]）。
    ...
    def score_topic(self, chart, full_chart: dict, topic: str,
        ...
        V2.7第一个版本: 主题→宫位映射 + 生年四化落宫 + 大限四化落宫(可选).

        Args:
            chart: compute()返回的ZiweiChart
            full_chart: full_chart()返回的完整命盘dict
            topic: 主题(婚姻/健康/财运/事业/官非/家庭/六亲/迁移/灾劫)
            lunar_date: 农历日期(如(1893,11,19), 用于计算生年干, 可选)
        ...
        """
        from .knowledge import THEME_TO_PALACE
        ...
        # 2.5 流年四化(V3.0: 按target_year计算流年天干四化, 影响本宫, 分值弱于大限)
        liunian_in = []
        if target_year:
            try:
        ...
        # 5. 宫干自化(V2.9.1: 自化力量弱于外来四化"自己给自己的"), 但仍有影响
    ...
    def flow_decadal_mutagen(self, years, lunar_date, hour, gender):
        ...
        # 转成 int keys, 否则大限四化全部失效（四重共振实验的大限层从未真正生效）
        return {int(k): v for k, v in data.items()}
    ...
    def flow_month_mutagen(self, year, month, lunar_date, hour, gender):
        ...
        《紫微斗数精成》: 看流月应以流年四化并配合大限之象来推——
        流月四化是紫微应期（具体到月份）的核心工具。iztro horoscope('Y-M-15')
        返回该阳历日所在农历月的流月四化（基于流月干支，斗数依流月命宫推算）。
    ...
    def corrected_hour_index(self, hour, longitude, solar_date):
        ...
        真太阳时 = 北京时间 + 经度差修正(+ (longitude - 120) × 4 分钟) + 均时差。
        经度差: (longitude - 120) × 4 分钟；均时差: NASA/Meeus 级数计算。
        hour: 出生钟表小时(北京时间, 0-23)。longitude: 出生地东经度数。
        solar_date: 出生阳历日期 (y,m,d)，用于均时差计算。
        ...
        try:
            ...
            from ..time.solar_time import calculate_true_solar_time
    ...
    def _stub(self, lunar_date, hour, gender):
        """Stub: derive main star deterministically from solar date.

        Real implementation calls iztro with lunar date.
        This stub keeps the architecture working for demo and unit tests.
        Self-contained: does NOT depend on tongshu-bazi package.
        """
        # Compute day stem from solar date using 60-day cycle.
        # Epoch: 1900-01-01 = Jia-Zi (index 0). lunar_python gives us solar back.
        try:
            from lunar_python import Solar
            solar = Solar.fromYmd(lunar_date[0], lunar_date[1], lunar_date[2])
            solar_back = solar
        except Exception:
            solar_back = Solar.fromYmd(1990, 5, 15)

        y, m, d = solar_back.getYear(), solar_back.getMonth(), solar_back.getDay()
        from datetime import date as _date
        days_since_epoch = (_date(y, m, d) - _date(1900, 1, 1)).days
        # 1900-01-01 was Jia-Zi (day_stem index 0 in 10-stem cycle)
        day_stem_idx = days_since_epoch % 10
        STEMS_PINYIN = ["JIA", "YI", "BING", "DING", "WU",
                        "JI", "GENG", "XIN", "REN", "GUI"]
        day_master = STEMS_PINYIN[day_stem_idx]

        # Element mapping (same as DAY_MASTER_ELEMENT in rules)
        DAY_MASTER_ELEMENT = {
            "JIA": "WOOD", "YI": "WOOD",
            "BING": "FIRE", "DING": "FIRE",
            "WU": "EARTH", "JI": "EARTH",
        ...
        element = DAY_MASTER_ELEMENT.get(day_master, "")

        # Map day_master -> representative main star
        star_map = {
            "JIA": "ZIWEI",    # 甲 - 紫微 (leadership) → SUPPORT
            "YI":  "TIANFU",   # 乙 - 天府 (stability) → SUPPORT
            "BING": "TAIYANG", # 丙 - 太阳 (light) → SUPPORT
            "DING": "TIANJI",  # 丁 - 天机 (intellect) → REFLECTION
            "WU":  "TIANFU",   # 戊 - 天府 (stability) → SUPPORT
            "JI":  "TIANLIANG", # 己 - 天梁 (elder care) → SUPPORT
            "GENG": "WUQU",    # 庚 - 武曲 (decision) → RESOURCE
            "XIN": "POJUN",    # 辛 - 破军 (change) → CHANGE
            "REN": "TAIYIN",   # 壬 - 太阴 (reflection) → REFLECTION
            "GUI": "JUMEN",    # 癸 - 巨门 (depth) → CONSTRAINT
        }
        main_star = star_map.get(day_master, "")
