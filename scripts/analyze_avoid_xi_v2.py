...
for m in mismatches:
    ...
    # 只分析引擎判喜的案例
    if not ('SUPPORT' in engine_label or '喜' in engine_label):
        continue
    ...
    try:
        ...
        # 只有忌神，没有用神
        if (gan_is_avoid or zhi_is_avoid) and not (gan_is_primary or zhi_is_primary):
            count += 1
            if count > 10:
                break
            ...
            print(f'  relations: {relations[:10]}')
        
    except Exception as e:
        print(f'  ERROR: {e}')

print(f'\n{"="*60}')
print(f'只有忌神但引擎判喜的案例总数: {count}')
