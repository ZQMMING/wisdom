...
print('==', ''.join(gz),'dm',ds,WUXING[ds],'月令',p['month'][1],'七档',sp['spectrum'],'ratio',round(sp.get('daymaster_ratio',0),3))
print('comb', {k:v for k,v in f['combination_facts'].items() if v})
for w in '木火土金水':
    d=wp['wuxing_power'][w]; print(' ',w,'ling',d.get('ling_state'),'ben',d.get('ben_n'),'zhong',d.get('zhong_n'),'yu',d.get('yu_n'),'stem',d.get('stem_n'),'ju',d.get('ju_n'),'detail',d.get('root_detail'))
for k in ('cong_type','cong_state','zhuanwang','zhuanwang_state','hua_qi','hua_qi_state','mu_mie','mu_mie_state','liangqi'):
    if spp.get(k): print(' spec',k,'=',spp.get(k))
print(' paths','/'.join(ye.get('yongshen_paths') or []))
print(' P',ye.get('yongshen_primary'),'S',ye.get('yongshen_secondary'),'A',ye.get('yongshen_avoid'))
