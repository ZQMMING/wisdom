...
exclude=['俗以','俗见','俗论','俗','财多','煞重','泄重','似乎','看似','弱中','弱变','弱不','非身弱','不论身','身弱者','徐注','注释','微论身强身弱','身弱有生必发','如夏贵妃造','身重','身强身弱之别','阴木归垣']
...
for r in rows:
    ...
    if '身弱' not in text:
        continue
    idx=text.find('身弱')
    context=text[max(0,idx-10):idx+len('身弱')+10]
...
for r in mismatch:
    ...
    print(f"身弱上下文: ...{r['judgment'][max(0,idx-50):idx+len('身弱')+50]}...")
