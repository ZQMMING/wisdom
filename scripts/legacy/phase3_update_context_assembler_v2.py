...
# 3. Update stem_ten_god assignments in assemble_natal_context
content = content.replace(
...
old_dayun_start = '''    def assemble_dayun_context(self, chart, natal: NatalContext, target_year: int) -> DaYunContext:
        """组装DaYunContext."""
...
new_dayun_start = '''    def assemble_dayun_context(self, chart, natal: NatalContext, target_year: int) -> DaYunContext:
...
content = content.replace(old_dayun_start, new_dayun_start)

# 6. Update assemble_year_context to use temp helpers
...
# 7. Add temp helper methods after __init__
...
print("Phase 3: ContextAssembler updated successfully")
