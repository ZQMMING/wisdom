# -*- coding: utf-8 -*-
"""P0-8.9整改版：真正的语义审计 - 19条FAIL逐条整改

核心原则（3f88d9e裁决冻结）:
1. 禁止通过字符串清洗制造PASS
2. min_truth必须100%由原文汉字组成，不能添加任何新字
3. 如果原典包含多个语义层，必须拆分成多个Assertion
4. 目标：让每条断言的结构真实符合原典
"""
...
class SemanticPuristAuditor:
    ...
    def load_failed_assertions(self, audit_filepath: str) -> List[dict]:
        ...
        failed = [r for r in data.get('audit_results', []) if r['status'] == 'FAIL']
        return failed
    
    def audit_semantic_purity(self, assertion: dict) -> dict:
        """真正的语义纯粹性审计"""
    ...
    def run_purist_audit(self, failed_assertions: List[dict]) -> dict:
        """运行真正的语义纯粹性审计"""
        ...
        for assertion in failed_assertions:
            result = self.audit_semantic_purity(assertion)
        ...
        print(f"  通过: {pass_count}条 ({pass_count/len(results)*100:.1f}%)")
        print(f"  需复审: {needs_review_count}条 ({needs_review_count/len(results)*100:.1f}%)")
...
def main():
    print("="*70)
    print("P0-8.9整改版：真正的语义审计 - 19条FAIL逐条整改")
    print("="*70)
    
    # 加载审计失败的断言
    audit_filepath = r'D:\shuntian\backend\data\p0_8_9_quality_audit.json'
    
    if not os.path.exists(audit_filepath):
        print(f"❌ 文件不存在: {audit_filepath}")
        return
    
    auditor = SemanticPuristAuditor()
    failed_assertions = auditor.load_failed_assertions(audit_filepath)
    
    # 运行审计
    results = auditor.run_purist_audit(failed_assertions)
    
    # 保存结果
    output_path = r'D:\shuntian\backend\data\p0_8_9_purist_audit.json'
    ...
    print(f"  通过: {results['pass_count']}条 ({results['pass_rate']:.1f}%)")
    print(f"  需复审: {results['needs_review_count']}条 ({100-results['pass_rate']:.1f}%)")
    ...
    if results['pass_count'] == len(results):
        print(f"P0-8.9 Purist Audit 🟢 PASS（所有断言语义纯粹）")
    else:
        print(f"P0-8.9 Purist Audit 🟡 HOLD（仍需整改{results['needs_review_count']}条）")
