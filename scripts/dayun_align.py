...
JI=['迁官','迁升','迁任','温饱','丰衣足食','颇丰','衣食丰','衣食颇丰','科甲连登','大魁天下','登科','登第','发甲','中乡榜','乡榜','南宫报捷','南宫','报捷','采芹','攀桂','月桂','撞破烟楼','高攀月桂','高躔','入泮','补廪','补禀','入词林','点中','仕版连登','仁版连登','连登甲第','连登','出仕','琴堂','知县','郡守','州牧','县令','封疆','发财','获利万金','获利','发福','发巨万','发财数万','发财十余万','大得际遇','得际遇','际遇','成家','立业','娶妻生子','生子','荫庇','荫疪','丰存','仕途坦平','坦平','顺遂','太平相业','宦海无波','无波','富贵','名利两全','名利','大好','丰盈','兴家','创业','起家','获一衿','可获一衿','一衿','秋闱有望','棘闱奏捷','奏捷','仕路','遂仕路','仕至','擢','丰厚','大利','事业巍峨','腾身','登月殿','月殿','琼林','庆集','云程','安享','其乐自如','琴书','家业日增','日增','病药相济','药病相济','有病得药','去病','吉','亨通','平宁','宁谧','升迁','举于乡','县宰','无恙']
XIONG=['罢职','削职','降职','丁忧','罢官','革职','家破身亡','破财而亡','家业破尽','破尽而亡','而亡','死于','死矣','病死','不禄','寿元有碍','丁艰','丁外艰','丁内艰','刑丧','刑伤','刑耗','刑克','克妻','克子','克夫','刑妻','刑耗并见','破财','破耗','耗散','家业渐消','家业消亡','家业耗散','家业破','家道日落','日落','蹭蹬','不捷','秋闱不捷','落职','诖误','大病','危险','犯事落职','贫乏不堪','贫困','贫乏','流为乞丐','乞丐','父母双亡','大凶','患难','病患','得病','血症','孤苦','削发为僧','一败如灰','倾家荡产','冻饿','嫖赌','祝融','骨肉之变','茕茕','只影','阻云程','一阻云程','多滞','少成','一败而尽','一败涂地','艰难']
...
def luck_verdict(txt,g,z):
    ...
    for st in sents:
        ...
        if '行'+g+z in st or '至'+g+z in st:
            hits.append(st); continue
        # V7.21: 扩展大运断语匹配格式——明确大运语境的格式
        # "岁运逢X"/"运至X"/"运逢X"/"大运X"/"运干X"/"运支X"格式
        if ('岁运逢'+g in st or '岁运逢'+z in st or
            '运至'+g in st or '运至'+z in st or
            '运逢'+g in st or '运逢'+z in st or
            '大运'+g in st or '大运'+z in st or
            '运干'+g in st or '运干'+z in st or
            '运支'+g in st or '运支'+z in st):
            hits.append(st); continue
    ...
    for _dy_idx,gz in enumerate(dy):
        ...
        if lc is None and _spec in ('衰极','太衰','衰') and gc=='fav' and zc!='fav':
            ...
            if (BRANCH_WX[z] in (KE.get(_dmwZ), _KEMEZ.get(_dmwZ))
                ...
                lc='av_l'
        # V5.1已移到lc计算之后(避免被合化等前置逻辑阻塞)
        # V5.3已沉淀到引擎层(yongshen_engine.py), 此处不再重复计算
