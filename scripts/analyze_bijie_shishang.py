...
for m in mismatches:
    ...
    # 只分析比劫+比劫和食伤+食伤
    if not (gan_wx == dm_wx and zhi_wx == dm_wx):
        # 食伤+食伤
        SHENG = {'木':'火','火':'土','土':'金','金':'水','水':'木'}
        if not (gan_wx == SHENG.get(dm_wx) and zhi_wx == SHENG.get(dm_wx)):
            continue
    ...
    try:
        ...
        combo_type = '比劫+比劫' if gan_wx == dm_wx else '食伤+食伤'
