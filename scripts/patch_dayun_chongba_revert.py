...
new="        clash=[c['verdict'] for c in transit_clash_verdicts(tp) if z in c['pair']]\n"
assert s.count(old)==1,('rev',s.count(old)); s=s.replace(old,new)
io.open(p,'w',encoding='utf-8',newline='').write(s)
print('冲拔修饰已回退(证伪: tier不含天干生扶/通关, 旺神发去病吉被误判)')
