...
# 在五合判断后增加天干五合月令判断
old = """        if wuhe_target and wuhe_target in original_stems:
            relations.append(f'GAN_WUHE_{wuhe_target}')
            if wuhe_huashen == primary:
                relations.append('WUHE_PRIMARY')  # 合化用神, 喜
        
        # V1.2: 三合判断"""
new = """        if wuhe_target and wuhe_target in original_stems:
            relations.append(f'GAN_WUHE_{wuhe_target}')
            if wuhe_huashen == primary:
                relations.append('WUHE_PRIMARY')  # 合化用神, 喜
        
        # V1.8: 天干五合月令判断 (大运天干与月令天干五合)
...
# 修改module版本号
c = c.replace("'module': 'DAYUN_XIJI_V1.7'", "'module': 'DAYUN_XIJI_V1.8'")

with open(p, 'w', encoding='utf-8', newline='') as f:
    f.write(c)
print('dayun_xiji.py V1.8优化完成(增加天干五合月令)')
