...
for m in mismatches:
    ...
    if len(chart) >= 4:
        mz = chart[3]
        if mz in ('卯', '酉'):
            mao_you_cases.append(m)
        else:
            other_cases.append(m)

print(f'卯酉月: {len(mao_you_cases)}, 其他月份: {len(other_cases)}')

# 分析前15个不匹配案例的用神/忌神
print(f'\n{"="*100}')
print('前15个不匹配案例用神/忌神分析:')
for i, m in enumerate(mismatches[:15]):
    ...
    try:
        ...
        # 跑大运喜忌
        dayun_list = [dayun_str] if dayun_str else []
