# -*- coding: utf-8 -*-
"""ZIPING V3.1 五经交叉验证 — 判据句级 (非关键词级).

升级要求:
- 不仅关键词出现, 必须包含判据结构 (用X/取X/X为Y/X不Y/X则Y/X得X/X即X/X者X/X必X)
- 排除目录页/总论页 (过短片段, < 50 字)
- 每条命中必须 ≥1 个判据结构词
- 找到 ≥1 个 判据句 → CONFIRMED; 多典 → CROSS-VERIFIED
"""
import json
from pathlib import Path

BASE = Path("D:/顺天系统资料/开发资料/参考资料/五经知识库/03_PASSAGES")
OUT = Path("D:/shuntian/docs/audit/ZIPING-V31-CROSS-VERIFICATION-v2.json")
...
def is_real_content(text):
    """排除目录/封面: 实际内容必须 ≥ 80 字 (目录页通常 < 100 字)。"""
    # Strip markdown
...
def search_book(book, keywords, max_results=5):
    ...
    for p in passages:
        ...
        if not is_real_content(text):
            continue
        # 必须有判据结构
        if not has_judgment_pattern(text):
            continue
        out.append({
...
# Cross-verification items with refined keywords
ITEMS = {
    "X1_得令定义": {
        "desc": "月支对日主生/同 = 得令",
        "PZZQ": ["得时", "得令", "失时", "月令"],
...
def cross_verify(item_id, desc, keywords_per_book):
    out = {}
    for book in BOOKS:
        ev = search_book(book, keywords_per_book.get(book, []))
        out[book] = {
            "count": len(ev),
            "evidence": ev[:2],
