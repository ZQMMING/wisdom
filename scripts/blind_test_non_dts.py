#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""盲测验证: 用非DTS案例(穷通宝鉴/子平真诠/渊海子平等)验证单一对齐率和命中率@K
目的: 确认69.4%单一对齐率是真实能力还是DTS过拟合
"""
import sys
sys.path.insert(0, '.')
...
# 读取all_cases.jsonl
all_cases = []
with open(r'D:\顺天系统资料\古书独立案例JSONL\all_cases.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
...
# 筛选非DTS案例
non_dts_cases = [d for d in all_cases if d.get('book') != '滴天髓']
print(f"\n非DTS案例数: {len(non_dts_cases)}")

# 解析八字
def parse_chart(chart_str):
    """解析八字字符串为四柱列表"""
    if not chart_str or len(chart_str) < 8: return None
    stems = chart_str[0::2][:4]
    branches = chart_str[1::2][:4]
...
def parse_dayun(dayun_str):
    """解析大运字符串为大运干支列表"""
    if not dayun_str: return []
    dayun = []
    for i in range(0, len(dayun_str)-1, 2):
        g = dayun_str[i]
        z = dayun_str[i+1]
...
for ci, case in enumerate(non_dts_cases):
    ...
    dayun_str = case.get('dayun','')
    judgment = case.get('judgment','')
    
    fp = parse_chart(chart)
    if not fp: continue
    dy = parse_dayun(dayun_str)
    ...
    for gz in dy:
        g, z = gz[0], gz[1]
        v, blob = luck_verdict(judgment, g, z)
...
if st['judgable']:
    ...
    if abs(_diff_single) < 10:
        print("  结论: 单一对齐率差异<10pp, 无显著过拟合")
    else:
        print("  结论: 单一对齐率差异>=10pp, 可能存在过拟合")
