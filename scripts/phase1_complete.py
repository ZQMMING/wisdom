...
def run():
    ...
    print("=== 检查现有表 ===")
    cr.execute("SELECT table_name FROM information_schema.tables WHERE table_schema='public' ORDER BY table_name")
    tables = [r[0] for r in cr.fetchall()]
    new_tables = ['life_domains','semantic_concepts','modern_expressions','tone_lexicon','forbidden_terms','risk_terms','templates','template_versions']
    existing = [t for t in new_tables if t in tables]
    ...
    for d in domains:
        ...
        """, d)
    print(f"  ✓ {len(domains)} life_domains")
    
    # semantic_concepts (10条)
    concepts = [
    ...
    for c in concepts:
        ...
        """, c)
    print(f"  ✓ {len(concepts)} semantic_concepts")
    ...
    for t in tones:
        ...
        """, t)
    print(f"  ✓ {len(tones)} tone_lexicon")
    ...
    for f in forbidden:
        ...
        """, f)
    print(f"  ✓ {len(forbidden)} forbidden_terms")
    
    # risk_terms (5条)
    risks = [
        ('RISK_01', 'probability_escalation', 'CERTAINTY_ESCALATION', 'WARN', '概率词升级（可能→很可能→一定）', '保持概率表述克制', None, 'ACTIVE'),
        ('RISK_02', 'modality_escalation', 'CERTAINTY_ESCALATION', 'WARN', '模态词升级（可以→应该→必须）', '尊重用户自主权', None, 'ACTIVE'),
        ('RISK_03', 'prediction_precision', 'CERTAINTY_ESCALATION', 'WARN', '预测精准化（趋势→精准时间）', '避免过度具体化', None, 'ACTIVE'),
        ('RISK_04', 'ancient_method_fabrication', 'AI_HALLUCINATION', 'WARN', '古法强引（无来源的"古法曰"）', '必须可追溯', None, 'ACTIVE'),
        ('RISK_05', 'insufficient_evidence_assertion', 'AI_HALLUCINATION', 'WARN', '证据不足断言', '双源核验原则', None, 'ACTIVE'),
    ]
    for r in risks:
        cr.execute("""
        ...
        """, r)
    print(f"  ✓ {len(risks)} risk_terms")
    
    # modern_expressions (6条)
    expressions = [
    ...
    for e in expressions:
        ...
        """, e)
    print(f"  ✓ {len(expressions)} modern_expressions")
    ...
    try:
        cr.execute("""
            ...
            VALUES ('20260821_translation_system_v1', NOW())
            ON CONFLICT (version) DO NOTHING
        """)
        print("  ✓ migration version recorded")
    except Exception as e:
        print(f"  ~ migration_versions skip: {e}")
    
    # 验证
    print("\n=== 验证 ===")
    for t in ['life_domains','semantic_concepts','modern_expressions','tone_lexicon','forbidden_terms','risk_terms','templates']:
        cr.execute(f'SELECT COUNT(*) FROM {t}')
        print(f'  {t}: {cr.fetchone()[0]} rows')
    
    cr.execute("SELECT COUNT(*) FROM semantic_mappings")
    print(f'  semantic_mappings: {cr.fetchone()[0]} rows')
    
    cr.execute("""
        SELECT column_name FROM information_schema.columns 
        WHERE table_name='semantic_mappings' AND column_name IN 
        ('concept_id','expression_refs','tone','priority','evidence_id','rule_id')
    """)
    cols = [r[0] for r in cr.fetchall()]
    print(f'  semantic_mappings 扩展列: {cols}')
