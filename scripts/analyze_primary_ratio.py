...
primary_ratios = []
for row in rows:
    # 从dayun_xiji字段中无法直接获取primary_power_ratio
    # 我们需要重新计算
    pass
...
from engines.common.l0_fact_builder import build_l0_facts

ratios = []
for i, row in enumerate(rows[:100]):  # 先分析前100个
    chart = row['chart']
    if not chart:
        continue
    pillars = {
