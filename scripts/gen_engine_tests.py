"""生成五部引擎骨架测试（修正版）：目录名/缩写/最小合法 chart 分离。"""
from pathlib import Path

ROOT = Path(r"D:\shuntian-ziping-p0")

ENGINES = [
    ("ziping_zhenquan", "ZIPING_ZHENQUAN", "pzzq"),
    ("ditiansui", "DITIANSUI", "dts"),
    ("qiongtong_baojian", "QIONGTONG_BAOJIAN", "qtbj"),
    ("sanming_tonghui", "SANMING_TONGHUI", "smth"),
    ("shenfeng_tongkao", "SHENFENG_TONGKAO", "sftk"),
]

TEMPLATE = '''"""Phase 1 Skeleton 测试（§60）· {dirname}。"""
...
for dirname, eid, reg in ENGINES:
    p = ROOT / "engines" / dirname / "tests" / "test_engine_skeleton.py"
    p.write_text(TEMPLATE.format(dirname=dirname, eid=eid, reg=reg), encoding="utf-8")
    print("OK", dirname)
print("done")
