...
class TestCanpingJintu:
    ...
    def test_search_dup_no_returns_all(self):
        """448 编号非唯一键：跨页异文应全部返回（p88/p89 三七二；p89/p90 三七五）。
        原 p61/p63 三一七、p61/p69 三一六 用例随豆丁互证编号修正（316->3116、317->3217、p63三一七待核）已失效，改用已定案跨页重出编号。"""
        hits = cp.search_raw_poem_448(no="372", part="tu")
        assert {h[1] for h in hits} == {88, 89}
        hits2 = cp.search_raw_poem_448(no="375", part="tu")
        assert {h[1] for h in hits2} == {89, 90}
