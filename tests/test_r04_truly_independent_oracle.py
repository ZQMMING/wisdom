...
# 2024年全年12节气精确时刻
LICHUN_2024    = datetime(2024, 2, 4, 16, 26, 53, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 立春 → 寅月
JINGZHE_2024   = datetime(2024, 3, 5, 16, 7, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))     # 惊蛰 → 卯月
QINGMING_2024  = datetime(2024, 4, 4, 15, 2, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))     # 清明 → 辰月
LIXIA_2024     = datetime(2024, 5, 5, 8, 10, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))     # 立夏 → 巳月
MANGZHONG_2024 = datetime(2024, 6, 5, 12, 10, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 芒种 → 午月
XIAOSHU_2024   = datetime(2024, 7, 6, 22, 20, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 小暑 → 未月
LISHU_2024     = datetime(2024, 8, 7, 8, 9, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))     # 立秋 → 申月
BAILU_2024     = datetime(2024, 9, 7, 11, 11, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 白露 → 酉月
HANLU_2024     = datetime(2024, 10, 8, 2, 59, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 寒露 → 戌月
LIDONG_2024    = datetime(2024, 11, 7, 6, 20, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 立冬 → 亥月
DAXUE_2024     = datetime(2024, 12, 6, 23, 17, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))  # 大雪 → 子月
XIAOHAN_2025   = datetime(2025, 1, 5, 10, 33, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))   # 小寒 → 丑月
...
# ============================================================================
# 完整12节气边界矩阵测试（12 × 3 = 36 cases）
# 每个节气：boundary-1秒 / boundary / boundary+1秒
# ============================================================================

# 12节气及其对应的精确时刻
SOLAR_TERM_BOUNDARIES_2024 = [
    ("立春", LICHUN_2024, "YIN"),       # 寅月
    ("惊蛰", JINGZHE_2024, "MAO"),      # 卯月
    ("清明", QINGMING_2024, "CHEN"),    # 辰月
    ("立夏", LIXIA_2024, "SI"),         # 巳月
    ("芒种", MANGZHONG_2024, "WU"),     # 午月
    ("小暑", XIAOSHU_2024, "WEI"),      # 未月
    ("立秋", LISHU_2024, "SHEN"),       # 申月
    ("白露", BAILU_2024, "YOU"),        # 酉月
    ("寒露", HANLU_2024, "XU"),         # 戌月
    ("立冬", LIDONG_2024, "HAI"),       # 亥月
    ("大雪", DAXUE_2024, "ZI"),         # 子月
    ("小寒", XIAOHAN_2025, "CHOU"),     # 丑月
]


def build_solar_terms_dict():
    """构建节气字典"""
    return {
        "LICHUN": LICHUN_2024,
        "JINGZHE": JINGZHE_2024,
        "QINGMING": QINGMING_2024,
        "LIXIA": LIXIA_2024,
        "MANGZHONG": MANGZHONG_2024,
        "XIAOSHU": XIAOSHU_2024,
        "LISHU": LISHU_2024,
        "BAILU": BAILU_2024,
        "HANLU": HANLU_2024,
        "LIDONG": LIDONG_2024,
        "DAXUE": DAXUE_2024,
        "XIAOHAN": XIAOHAN_2025,
    }


def build_boundary_matrix():
    """构建 12×3 边界矩阵（36 cases）"""
    test_cases = []
    
    # 完整节气循环：寅卯辰巳午未申酉戌亥子丑
    branch_cycle = ["YIN", "MAO", "CHEN", "SI", "WU", "WEI", "SHEN", "YOU", "XU", "HAI", "ZI", "CHOU"]
    
    for i, (term_name, term_dt, expected_branch) in enumerate(SOLAR_TERM_BOUNDARIES_2024):
        # 前一个节气对应的月支
        prev_branch = branch_cycle[(i - 1) % 12]
        
        # boundary-1秒 → 仍属前一个月
        test_cases.append((term_dt + timedelta(seconds=-1), term_name, prev_branch, "前1秒"))
        # boundary → 属于本月
        test_cases.append((term_dt, term_name, expected_branch, "时刻"))
        # boundary+1秒 → 属于本月
        test_cases.append((term_dt + timedelta(seconds=1), term_name, expected_branch, "后1秒"))
...
def verify_independent_pillars():
    ...
    solar_terms = build_solar_terms_dict()
    boundary_matrix = build_boundary_matrix()
    
    print("=" * 70)
    print("R-04 Independent Bazi Formula Verification")
    print("Full 12-Solar-Term Boundary Matrix (36 cases)")
    print("=" * 70)
    
    # ===================== 测试1：12×3 边界矩阵（月柱分支验证）=====================
    print("\n--- Test 1: 12 Solar-Term × 3 Second Boundary Matrix (Month Branch) ---")
    for civil_dt, term_name, expected_branch, label in boundary_matrix:
        civil_date = civil_dt.date()
        effective_date = compute_effective_date(civil_dt)
        effective_hour = compute_effective_hour(civil_dt)
        pre_lichun = civil_dt < LICHUN_2024
        
        year_stem, year_branch = compute_year_pillar(civil_date.year, pre_lichun)
        month_stem, month_branch = compute_month_pillar(year_stem, civil_dt, solar_terms)
        
        branch_ok = month_branch == expected_branch
        status = "PASS" if branch_ok else "FAIL"
        
        print(f"{status} {term_name}{label}: month_branch={month_branch}, expected={expected_branch}")
        results.append(branch_ok)
    
    # ===================== 测试2：立春精确时刻边界（秒级）=====================
    print("\n--- Test 2: Lichun Second-Precision Boundary ---")
    lichun_tests = [
    ...
    for civil_dt, exp_year, exp_month, exp_day, exp_hour, desc in lichun_tests:
        civil_date = civil_dt.date()
        effective_date = compute_effective_date(civil_dt)
        effective_hour = compute_effective_hour(civil_dt)
        pre_lichun = civil_dt < LICHUN_2024
        ...
        if not all_ok:
            if not y_ok: print(f"   Year: expected={exp_year[0]} {exp_year[1]}, got={year_stem} {year_branch}")
            if not m_ok: print(f"   Month: expected={exp_month[0]} {exp_month[1]}, got={month_stem} {month_branch}")
            if not d_ok: print(f"   Day: expected={exp_day[0]} {exp_day[1]}, got={day_stem} {day_branch}")
            if not h_ok: print(f"   Hour: expected={exp_hour[0]} {exp_hour[1]}, got={hour_stem} {hour_branch}")
        
        results.append(all_ok)
    
    # ===================== 测试3：23:00 换日边界 =====================
    print("\n--- Test 3: 23:00 Day-Boundary ---")
    boundary_23_tests = [
    ...
    for civil_dt, exp_year, exp_month, exp_day, exp_hour, desc in boundary_23_tests:
        civil_date = civil_dt.date()
        effective_date = compute_effective_date(civil_dt)
        effective_hour = compute_effective_hour(civil_dt)
        pre_lichun = civil_dt < LICHUN_2024
    ...
    # ===================== 测试4：时辰边界 =====================
    print("\n--- Test 4: 12 Hour-Branch Coverage ---")
    hour_tests = [
        # 2024-02-04 12:00: 戊日，午时(6)→戊午
        (datetime(2024, 2, 4, 12, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai")),
         ("GUI", "MAO"), ("YI", "CHOU"), ("WU", "XU"), ("WU", "WU"), "午时12:00"),
        # 2024-02-04 00:30: 戊日，子时(0)→壬子
        (datetime(2024, 2, 4, 0, 30, 0, tzinfo=ZoneInfo("Asia/Shanghai")),
         ("GUI", "MAO"), ("YI", "CHOU"), ("WU", "XU"), ("REN", "ZI"), "子时00:30"),
        # 2024-02-04 03:30: 戊日，寅时(4)→甲寅
        (datetime(2024, 2, 4, 3, 30, 0, tzinfo=ZoneInfo("Asia/Shanghai")),
         ("GUI", "MAO"), ("YI", "CHOU"), ("WU", "XU"), ("JIA", "YIN"), "寅时03:30"),
        # 2024-02-04 15:30: 戊日，申时(8)→庚申
        (datetime(2024, 2, 4, 15, 30, 0, tzinfo=ZoneInfo("Asia/Shanghai")),
         ("GUI", "MAO"), ("YI", "CHOU"), ("WU", "XU"), ("GENG", "SHEN"), "申时15:30"),
    ]
    
    for civil_dt, exp_year, exp_month, exp_day, exp_hour, desc in hour_tests:
        civil_date = civil_dt.date()
        effective_date = compute_effective_date(civil_dt)
        effective_hour = compute_effective_hour(civil_dt)
        pre_lichun = civil_dt < LICHUN_2024
...
if __name__ == "__main__":
    ...
    if ok:
        ...
        print("12 Solar-Term × 3 Boundary Matrix verified")
