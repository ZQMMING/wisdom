...
# 未注册: 庚金等仍 NOT_REGISTERED
r_no = build_climate_candidates(build(mk('辰', day_gz=('庚', '午'))))  # 庚日辰月
check('庚金未注册->NOT_REGISTERED', r_no['state'] == 'NOT_REGISTERED')
check('庚金未注册->空候选', r_no['candidate_count'] == 0 and r_no['climate_candidates'] == [])
