# 1980-06-22 10:00 男 广州 · 盲派全链路 L3 解析层输出（引擎原始输出，未经 LLM 加工）

> 链路：L0 八字排盘 → L1 做功 → L1e 事件 → L1f 应期 → L2 断语 → L2.5 12主题聚合 → L3 原文+现代语义

## [THEME-001] 性情禀赋 · ESTABLISHED
- 综合：性情由旺衰+五行+透干十神共同决定，以下为逐项事实（盲派十神心性）
  - 事实 blind_wangshuai=WANG
    原文：盲派四定律·身旺以官杀（含纳音）
    现代语义：日主自身力量旺，扛得住财官，做事有底气
  - 事实 five_element_imbalance=TRUE
    原文：五行失衡（排盘层五行统计）
    现代语义：五行分布不均，性情有偏向，某方面特质突出
  - 事实 transparent_ten_gods={'year': '偏财', 'month': '七杀', 'hour': '正官'}
    原文：盲派十神心性（透干十神）
    现代语义：见逐项十神性情（entry 内扩展）
  - 事实 transparent_ten_gods.year=偏财
    原文：十神心性·偏财：慷慨大方，重实际，善经营
    现代语义：透干偏财：为人慷慨大方，注重实际利益，善于经营谋划
  - 事实 transparent_ten_gods.month=七杀
    原文：十神心性·七杀：果断威权，性急多疑
...
- 综合：身体以禄神/羊刃/燥土状态为据（盲派身体章）
  - 事实 body_event_candidate.candidate=LU_UNDER_ATTACK
    原文：盲派口诀·禄怕见绝更怕穿害（案例集戊申己未庚申辛巳交通意外）
    现代语义：禄神受攻击（被穿/合克/脆金），身体或福报易受损
  - 事实 body_event_candidate.lu_attacked=True
    原文：盲派口诀·禄怕见绝更怕穿害
    现代语义：禄神状态受损（穿/冲/合克）
  - 事实 dry_earth_brittle=NO_DRY_EARTH
    原文：盲派口诀·燥土脆金（VERIFY-BLIND-034）
    现代语义：无燥土脆金之患

## [THEME-007] 迁移出行 · ESTABLISHED
- 综合：迁移以驿马+引动状态为据（盲派金口诀·论驿马）
  - 事实 yima.present=SHEN马在YIN_AND_YIN马在SHEN
    原文：盲派金口诀·论驿马：寅午戌马在申、申子辰马在寅
    现代语义：命带驿马（申马在寅、寅马在申），主走动奔波
  - 事实 yima.trigger=NO_TRIGGER
    原文：盲派金口诀·驿马引动
    现代语义：驿马未被大运流年冲合引动，暂不迁移

## [THEME-008] 事业功名 · ESTABLISHED
- 综合：事业以做功类型+官杀制净状态为据（盲派职业/官贵章）
  - 事实 occupation_candidate.work_types=['CONTROL_OFFICER_BY_FOOD_INJURY', 'CONTROL_WEALTH_BY_BIJIE', 'CONTROL_WEALTH_BY_INTERACTION', 'CONTROL_OFFICER_BY_INTERACTION', 'GENERATE_WEALTH_BY_FOOD_INJURY', 'TRANSFORM_OFFICER_BY_RESOURCE', 'CONTROL_FOOD_INJURY_BY_RESOURCE', 'CONTROL_BIJIE_BY_OFFICER']
    原文：案例12：伤食制官局，命有官职
    现代语义：以食伤制官杀取功名，走公职/管理路线
...
- 综合：才艺学业以印做功+食伤泄秀+文理方向为据（段建业《盲派中级命理学》第11章）
  - 事实 zuo_gong.印做功(学业)=EFFECTIVE
    原文：段建业《盲派中级命理学》第11章：印星须做功方表学历
    现代语义：印星做功成立，学业有成就
  - 事实 zuo_gong.食伤泄秀(才艺)=NOT_EFFECTIVE
    原文：段建业《盲派中级命理学》第11章
    现代语义：食伤泄秀不成立，才艺表现平平
  - 事实 talent.direction=WEN(木火)
    原文：段建业《盲派中级命理学》第11章：金水主理，木火主文
    现代语义：文理方向偏文（木火）

## 事件层（L2→L3）
- [MARRIAGE] MARRIAGE_BROKEN（IN_AUSPICIOUS）
    原文：盲派婚姻篇·配偶宫逢冲必离婚/配偶宫破星损（BLIND-DJ-004）
    现代语义：婚姻容易出现分离、难长久的问题
- [WEALTH] WEALTH_ESTABLISHED（AUSPICIOUS）
    原文：盲派财富章·财现+财被取+做功成（BLIND-DJ-005/007/009）
    现代语义：求财有成，财富能到手
- [OFFICIAL] OFFICIAL_ESTABLISHED（AUSPICIOUS）
    原文：盲派官贵章·制尽杀星得天下（BLIND-DJ-006）
    现代语义：功名/管理有成，能掌权
- [OCCUPATION] OCCUPATION_DIRECTION_CANDIDATE（NEUTRAL）
    原文：盲派职业章·做功类型映射职业候选
    现代语义：职业方向候选（见 occupation_name）
- [BODY] BODY_LU_ATTACK（IN_AUSPICIOUS）
    原文：盲派口诀·禄怕见绝更怕穿害（BLIND-DJ-002）
    现代语义：禄神受攻击，身体或福报易受损