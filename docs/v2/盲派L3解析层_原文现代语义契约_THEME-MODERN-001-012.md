...
2. **出处必须可追溯（古籍/口诀原文，案例不入注册表）**：每条 original 带出处（书名《》/篇名盲派X章/
   ...
   **盲派引擎全域（代码+注释）禁止出现任何验证样本引用**——验证样本只存在于回归测试（tests/），
   不参与规则依据；引擎内规则出处只允许古籍/口诀/VERIFY 号/段建业原书。
...
- 验证样本清理记录（2026-09-13）：盲派引擎全域（5 个引擎文件，含代码注释）清零验证样本引用——
  blind_bazi_engine.py 42 处注释（原书引文/包局命例/官杀核证/职业映射等）全部改写为古籍/口诀/
  VERIFY 号出处；blind_judgment.py/blind_yingqi.py/blind_interpretation.py 同步清理；出处审计
  函数移除"案例"合法出处标记。清理后盲派引擎全域"案例"残留=0，出处审计 PASS
  （130 取证 + 11 声明未取证 + 0 无出处），modern 忠实 PASS，全集 FULL，86 测试全绿，
  16 例回归 MISSING=0。
