...
**✅ 契约完整**：API 接收 hour + optional minute
...
def bazi_view(self) -> tuple[int, int, int, int]:
    return (
        ...
        self.effective_hour,  # ← 🔴 只返回 hour，minute 丢失
    )
```
...
**🔴 问题 P5：大运循环只有 range(1, 4)，只生成3个大运，应为10个**
...
### P0-1：修复 _compute_with_sxtwl 接受 minute
...
**根本问题**：时间链在 Adapter 层断裂

```
TimeResolver  ✅ 保留完整时间（含分秒）
    ↓
CalculationContext  ✅ 有 effective_minute + solar_datetime
    ↓
bazi_view  🔴 只返回 hour，minute 丢失
    ↓
BaziEngine._compute_with_sxtwl  🔴 硬编码 t.m=0
```

**修复路径**：
1. 修复 `_compute_with_sxtwl` 接受 minute/second 参数
2. 修复 `BaziAdapter` 传递完整 `solar_datetime`
3. 修复 `CanonicalBaziChart` 添加 `birth_datetime` 字段
4. 修复大运循环为 range(1, 11)
5. 补充分钟级边界测试

完成以上修复后，再进行 Calculation Freeze Gate。
